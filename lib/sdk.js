...
function setApiToken(server_opts, api_token) {
	return new Promise(function(resolve, reject) {
		fs.readFile(api_tokens_file, function(error, body) {
			...
			fs.writeFile(api_tokens_file, JSON.stringify(api_tokens), function(error) {
...
function request(server_opts, method, data) {
	...
	return Promise.all([read_api_token_if_necessary, getSDKVersion()])
		.then(([api_token, sdk_version]) => new Promise(function(resolve, reject) {
			let url = "https://" + server_opts.host + "/api/v1/" + method;
			if (server_opts.host.match(/^(localhost|127\.0\.0\.1|.*\.local)(:\d+)?$/)) {
				url = "http://" + server_opts.host + "/api/v1/" + method;
			}
			let request_params = {
				method: "POST",
				uri: url,
			...
			data = Object.assign(data, { api_token, sdk_version });
			if (server_opts.user) {
				request_params.auth = {
...
function runBuildCommand(template_dir, command) {
	...
	return new Promise(function(resolve, reject) {
		...
		try {
			child_process.spawn(prog, args, { cwd: template_dir, stdio: "inherit" })
	...
	return readConfig(template_dir)
		.then((config) => {
			...
			for (let key in config.build) {
				build_rules.push(Object.assign(config.build[key], { key }));
			}
			return build_rules;
		});
}
...
TEMPLATE_SPECIAL_DIRECTORIES = new Set([
	"static", "data",
]),
TEMPLATE_SPECIAL = new Set([
	"index.html", "template.js", "template.yml", "thumbnail.png", "thumbnail.jpg", "README.md",
	"static", "data",
]);

module.exports = {
	checkTemplateVersion, getSDKMajorVersion,

	getApiToken, setApiToken, deleteApiTokens,
	request,
	runBuildCommand, buildTemplate,
	readConfig, writeConfig, buildRules,
