...
function zipUpTemplate(template_dir, config) {
	return new Promise(function(resolve, reject) {
		tmp.file({ keep: true }, function(error, zip_filename, zip_fd) {
			...
			zip.append(JSON.stringify({
				...
				autoheight: JSON.stringify(config.autoheight),
...
function publish(args, server_opts) {
	...
	(args.build ? sdk.buildTemplate(template_dir) : Promise.resolve())
