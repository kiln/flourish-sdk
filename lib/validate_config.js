...
const log = require("./log"),
      columns = require("../server/columns");
...
const VALID_SETTING_TYPES = new Set(["number", "string", "text", "code", "boolean", "color", "url"]);
function validateSetting(conditional_settings, setting) {
...
function validateSettings(settings, bindings) {
	...
	for (let setting of settings) {
		...
		validateSetting(conditional_settings, setting);
		...
		conditional_settings.forEach(function(conditional_setting) {
			if (/^data./.test(conditional_setting)) {
				if (!/^data.\w+.\w+$/.test(conditional_setting)) {
					throw new Error(`template.yml: “show_if” or “hide_if” property specifies invalid data binding “${conditional_settings}”`);
				}
...
function validateBreakpointVal(value, name) {
	...
	if (value !== "auto" && !/^(0|\d+px)$/.test(value) && !/^\d+x\d+$/.test(value)) {
...
function validateConfig(config, data_directory) {
	...
	if ("settings" in config) validateSettings(config.settings, config.data);
}
