...
const columns = require("../common/utils/columns");
...
function validateSetting(template_directory, conditional_settings, setting) {
	...
	if ("cacheable_for_standalone_downloads" in setting) {
		if (setting.type !== "url") {
			throw new Error(`template.yml setting “${property}” has “cacheable_for_standalone_downloads” property, but is not of type “url”`);
		}
		if (typeof setting.cacheable_for_standalone_downloads !== "boolean") {
			throw new Error(`template.yml setting “${property}” has an invalid value for “cacheable_for_standalone_downloads”: should be true or false`);
		}
	}
}
...
function validateDataBinding(binding, data_table_names) {
	...
	if ("cacheable_for_standalone_downloads" in binding) {
		if (typeof binding.cacheable_for_standalone_downloads !== "boolean") {
			throw new Error(`template.yml “cacheable_for_standalone_downloads” property of data binding “${binding_name}” must be a boolean`);
		}
	}
}
...
function validateIsPremium(is_premium) {
	if (typeof is_premium !== "boolean") { throw new Error(`template.yml: Bad is_premium setting; must be either true or false`); }
}
...
function validateAllowedStandaloneDownloadOrigins(origins) {
	if (!Array.isArray(origins)) {
		throw new Error("template.yml: “allowed_standalone_download_origins” must be an array");
	}
	origins.forEach(function(origin) {
		if (typeof origin !== "string") {
			throw new Error(`template.yml: allowed_standalone_download_origins entry “${origin}” is not a string`);
		}
		// Basic URL validation - should start with http:// or https://
		if (!origin.startsWith("http://") && !origin.startsWith("https://")) {
			throw new Error(`template.yml: allowed_standalone_download_origins entry “${origin}” must be a valid HTTP(S) URL`);
		}
	});
}
...
function validateConfig(config, template_directory) {
	...
	if ("is_premium" in config) { validateIsPremium(config.is_premium); }
	...
	if ("allowed_standalone_download_origins" in config) {
		validateAllowedStandaloneDownloadOrigins(config.allowed_standalone_download_origins);
	}
}
